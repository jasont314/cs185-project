"""PPO agent with step-conditioned value function V(s, k).

A simpler alternative to intra-chain credit assignment that conditions
the intra-chain value function only on (state, step_index), NOT on the
latent z_k.  This avoids the cold-start problem because the value
estimate does not depend on near-random latents early in training.

Per-step advantages are still computed via temporal differencing:

    A_{t,k} = V(s, k-1) - V(s, k)

but now the value function sees only the state and step index.
"""
...
from src.models.value_function import StepConditionedValueFunction, ValueFunction
...
_DEFAULT_CONFIG: Dict[str, Any] = {
    ...
    "max_grad_norm": 0.5,
    "target_kl": None,
    # Step-conditioned specific
    "intra_vf_coef": 0.5,
    "intra_vf_lr": 3e-4,
    "intra_hidden_dim": 128,
    # Training
    "num_steps": 2048,
    "num_epochs": 10,
    "batch_size": 64,
...
class PPOStepConditioned:
    """PPO agent with step-conditioned intra-chain value function.

    Uses V(s, k) instead of V(s, z_k, k) -- no latent conditioning.
    ...
    def __init__(self, config: Dict[str, Any]) -> None:
        ...
        # Step-conditioned value function V(s, k) -- no latent input
        self.intra_vf = StepConditionedValueFunction(
            state_dim=self.cfg["state_dim"],
            K=K,
    ...
    def update(self) -> Dict[str, float]:
        ...
        for epoch in range(self.cfg["num_epochs"]):
            for batch in self.buffer.get_batches(self.cfg["batch_size"]):
                ...
                # Compute intra-chain per-step advantages using V(s, k)
                # Key difference: no latents argument!
                intra_adv = self.intra_vf.compute_intra_advantages(
                    states, advantages
                ).detach()

                # Per-step policy loss using intra-chain advantages
                ...
                # Step-conditioned value function loss -- no latents!
                intra_vf_loss = self.intra_vf.compute_loss(states, advantages)
    ...
    def _compute_intra_policy_loss(
        ...
        """PPO clipped loss with step-conditioned intra-chain advantages.
