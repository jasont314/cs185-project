"""PPO with hierarchical advantage + cumulative-product ratio clipping.

Combines:
  - Hierarchical advantages: A_k = A/K + delta_k(s, z_k, k)
    where delta sums to 0, preserving the holistic advantage while learning
    non-uniform credit redistribution across flow steps.
...
from src.algorithms.losses import (
    ...
    compute_hierarchical_cumulative_loss,
    compute_value_loss,
)
...
class PPOHierarchicalCumulative:
    """PPO agent combining hierarchical advantage with cumulative ratio clipping.

    Uses a correction network to learn zero-mean per-step adjustments on top
    of the uniform baseline A_t/K, and clips the cumulative product of
    importance ratios rather than each per-step ratio independently.
    """
    ...
    def update(self) -> Dict[str, float]:
        ...
        for epoch in range(self.cfg["num_epochs"]):
            for batch in self.buffer.get_batches(self.cfg["batch_size"]):
                ...
                # Compute corrections (gradients flow through to train correction_net)
                delta = self.correction_net(states, latents, K)

                # Hierarchical + cumulative PPO loss
                policy_loss, info = compute_hierarchical_cumulative_loss(
                    new_pslp, old_pslp, advantages, delta, K,
                    clip_eps=effective_clip_eps,
                    delta_reg=self.cfg["delta_reg"],
